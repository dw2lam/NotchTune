...
/// Installs/uninstalls NotchTune's managed `[[hooks]]` entries in
/// `~/.kimi/config.toml`.
///
...
public enum KimiHookInstaller {
    ...
    private static func blockMatchesManagedCommand(_ block: [String], managedCommand: String?) -> Bool {
        for line in block {
            ...
            if isLegacyNotchTuneHookCommand(value) {
                return true
            }
        }
        return false
    ...
    /// escape handling is unnecessary — NotchTune only ever writes paths,
