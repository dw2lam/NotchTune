...
///
/// Uses `NWListener` for TCP + Bonjour advertising of `_notchtune._tcp`.
...
public final class WatchHTTPEndpoint: @unchecked Sendable {
    private static let logger = Logger(subsystem: "app.notchtune", category: "WatchHTTPEndpoint")
    private static let serviceType = "_notchtune._tcp"
    private static let pairingCodeLength = 4
    private static let pairingCodeExpiry: TimeInterval = 120 // 2 minutes

    private let queue = DispatchQueue(label: "app.notchtune.watch.http", qos: .userInitiated)
