...
public struct ClaudeStatusLineInstallationStatus: Equatable, Sendable {
    ...
    /// the user's existing `statusLine.command` under `_notchTuneOriginalStatusLine`.
...
public let notchTuneOriginalStatusLineKey = "_notchTuneOriginalStatusLine"
/// Key written by pre-rename installs; still honored when restoring/uninstalling.
public let legacyOriginalStatusLineKey = "_openIslandOriginalStatusLine"

public final class ClaudeStatusLineInstallationManager: @unchecked Sendable {
    public static let managedScriptName = "notchtune-statusline"
    public static let wrappedDelegateScriptName = "notchtune-statusline-delegate"
    public static let legacyManagedScriptName = "open-island-statusline"
    ...
    public func status() throws -> ClaudeStatusLineInstallationStatus {
        ...
        let managedStatusLineIsWrapper = managedStatusLineConfigured
            && (settings[notchTuneOriginalStatusLineKey] != nil
                || settings[legacyOriginalStatusLineKey] != nil)
    ...
    public func installAsWrapper() throws -> ClaudeStatusLineInstallationStatus {
        ...
        if let originalStatusLine = mutatedSettings["statusLine"] {
            mutatedSettings[notchTuneOriginalStatusLineKey] = originalStatusLine
        }
        mutatedSettings["statusLine"] = managedStatusLine(for: scriptURL)
    ...
    public func uninstall() throws -> ClaudeStatusLineInstallationStatus {
        ...
        if currentStatus.managedStatusLineConfigured {
            ...
            if let savedOriginal = settings[notchTuneOriginalStatusLineKey]
                ?? settings[legacyOriginalStatusLineKey] {
                settings["statusLine"] = savedOriginal
                settings.removeValue(forKey: notchTuneOriginalStatusLineKey)
                settings.removeValue(forKey: legacyOriginalStatusLineKey)
            } else {
    ...
    public static func wrappedDelegateScript(originalCommand: String) -> String {
        "#!/bin/bash\n# Original Claude Code statusLine.command preserved by NotchTune.\n\(originalCommand)\n"
    }
    ...
    public static func managedScript(cacheURL: URL = managedCacheURL) -> String {
        ...
        # It feeds usage data (5h/7d quotas) to the NotchTune
