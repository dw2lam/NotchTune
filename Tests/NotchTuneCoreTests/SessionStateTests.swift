...
struct SessionStateTests {
    ...
    func actionableStateResolvedClearsWaitingForApproval() {
        ...
        #expect(state.session(id: "claude-approval")?.summary == "Approval was handled outside NotchTune.")
    }
    ...
    func codexPreToolUseWaitsForApprovalAndReturnsDenyDirective() async throws {
        ...
        let response = try await responseTask
        #expect(response == .codexHookDirective(.deny(reason: "Permission denied in NotchTune.")))
    }
    ...
    @Test
    func staleCodexApprovalClickAfterHookDisconnectDoesNotMutateSessionState() async throws {
        let socketURL = BridgeSocketLocation.uniqueTestURL()
        let server = BridgeServer(socketURL: socketURL)
        try server.start()
        defer { server.stop() }
        ...
        let payload = CodexHookPayload(
            ...
            model: "gpt-5-codex",
            permissionMode: .default,
            sessionID: "codex-permission-stale",
            transcriptPath: nil,
            turnID: "turn-1",
            toolName: "Bash",
        ...
        let disconnectedEvent = try await nextEvent(from: &iterator)
        #expect(disconnectedEvent.actionableResolution?.summary == "Hook process disconnected.")

        try await observer.send(.resolvePermission(sessionID: "codex-permission-stale", resolution: .allowOnce()))

        let staleClickEvent = try await nextEvent(from: &iterator)
        #expect(staleClickEvent.actionableResolution?.summary == "Permission request is no longer active.")
    }
    ...
    func codexHookInstallerMergesManagedGroupsWithoutDroppingUnrelatedHooks() throws {
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            ...
            hookCommand: "'/tmp/NotchTuneHooks'"
        )
        ...
        let managedStopHook = stopGroups?
            .compactMap { $0["hooks"] as? [[String: Any]] }
            .flatMap { $0 }
            .first(where: { $0["command"] as? String == "'/tmp/NotchTuneHooks'" })

        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/NotchTuneHooks'"))
    ...
    @Test
    func codexHookInstallerInstallReplacesLegacyNotchTuneCommands() throws {
        let existing = """
        {
          "hooks": {
            "PreToolUse": [
            ...
            "Stop": [
              {
                "hooks": [
                  ...
                  {
                    "type": "command",
                    "command": "'/tmp/old-debug/NotchTuneHooks'"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true"
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            existingData: existing,
            hookCommand: "'/tmp/new-release/NotchTuneHooks'"
        )
        ...
        #expect(permissionCommands == ["'/tmp/new-release/NotchTuneHooks'"])
        #expect(hooks?["PostToolUse"] == nil)
        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/new-release/NotchTuneHooks'"))
        #expect(!stopCommands.contains("'/Users/test/.open-island/bin/open-island-bridge' --source codex"))
        #expect(!stopCommands.contains("'/tmp/old-debug/NotchTuneHooks'"))
    }
    ...
    func codexHookInstallerUninstallRemovesOnlyManagedHooks() throws {
        ...
        let mutation = try CodexHookInstaller.uninstallHooksJSON(
            existingData: existing,
            managedCommand: "'/tmp/NotchTuneHooks'"
        )
    ...
    func codexHookInstallationManagerRoundTripsInstallAndUninstall() throws {
        ...
        let managedHooksBinaryURL = rootURL
            ...
            .appendingPathComponent("NotchTuneHooks")
...
private extension AgentEvent {
    ...
    var actionableResolution: ActionableStateResolved? {
...
private func sendOnGCDThread(
    ...
    try await withCheckedThrowingContinuation { continuation in
        DispatchQueue.global().async {
            do {
                let response = try BridgeCommandClient(socketURL: socketURL).send(command, timeout: timeout)
