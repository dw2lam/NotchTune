...
```
I'm having an issue with Open Island (https://github.com/dw2lam/NotchTune).
...
1. Collect my environment info:
   ...
   - Check if NotchTune is running: `ps aux | grep -i "open.island\|NotchTuneApp" | grep -v grep`
...
```
I'd like to request a feature for Open Island (https://github.com/dw2lam/NotchTune).
...
NotchTune is a native macOS companion app for AI coding agents. It sits in the notch/top-bar area and monitors local agent sessions, surfaces permission requests, answers questions, and provides "jump back" to the correct terminal context. Local-first, no server dependency.
...
swift build -c release --product NotchTuneHooks
```
